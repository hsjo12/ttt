...
async function main() {
  ...
  */  
  ...
  const SortedProposalList = await hre.ethers.getContractFactory("SortedProposalList");
  const sortedProposalList = await SortedProposalList.deploy()
  await sortedProposalList.deployed();

  const SortedQUEUElList = await hre.ethers.getContractFactory("SortedQueueAndExecutionList");
  const sortedQUEUElList = await SortedQUEUElList.deploy()
  await sortedQUEUElList.deployed();

  const ProposalListUpKeep = await hre.ethers.getContractFactory("ProposalListsUpKeep");
  const proposalListUpKeep = await ProposalListUpKeep.deploy(sortedProposalList.address)
  await proposalListUpKeep.deployed();

  const QueueListUpKeep = await hre.ethers.getContractFactory("QueueAndExecutionListsUpKeep");
  const queueListUpKeep = await QueueListUpKeep.deploy(sortedQUEUElList.address)
  await queueListUpKeep.deployed();

  const SORTED_PROPOSAL_ADDRESS = sortedProposalList.address;
  const SORTED_QUEUE_AND_EXECUTION = sortedQUEUElList.address;
  ...
  CreateJs(sortedProposalList, "SortedProposalList", chainId);
  CreateJs(sortedQUEUElList, "SortedQueueAndExecutionList", chainId);
  CreateJs(proposalListUpKeep, "ProposalListsUpKeep", chainId);

  CreateJs(queueListUpKeep, "QueueAndExecutionListsUpKeep", chainId);
